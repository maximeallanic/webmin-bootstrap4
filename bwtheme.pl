...
sub theme_header {
	...
	print '<link href="'. $gconfig{'webprefix'} . '/unauthenticated/assets/css/bootstrap-material-design.min.css" rel="stylesheet" type="text/css">' , "\n";
	print '<link href="'. $gconfig{'webprefix'} . '/unauthenticated/assets/css/circleprogress.css" rel="stylesheet" type="text/css">' , "\n";
	print '<link href="'. $gconfig{'webprefix'} . '/unauthenticated/assets/css/default.css" rel="stylesheet" type="text/css">' , "\n";
	...
	print '<script src="'. $gconfig{'webprefix'} . '/unauthenticated/assets/js/popper.min.js" type="text/javascript"></script>' , "\n";
	...
	print '<script src="'. $gconfig{'webprefix'} . '/unauthenticated/assets/js/bootstrap-material-design.min.js" type="text/javascript"></script>' , "\n";
	print '</head>' , "\n";
	print '<body class="d-flex flex-column">' , "\n";
...
if (@_ > 1) {
	...
	print "<table class='header my-5' width=100%><tr>\n";
	...
	if ($_[1]) {
		...
		print "<img alt=\"$_[0]\" src=\"$_[1]\">\n";
		}
	else {
		# Title is just text
		my $ts = defined($tconfig{'titlesize'}) ?
				$tconfig{'titlesize'} : "+2";
		print ($ts ? "<font size=$ts>" : ""),$_[0],
		      ($ts ? "</font>" : "");
		print "<br>$_[9]\n" if ($_[9]);
		}
...
sub theme_generate_icon {
	...
	if ($tconfig{'noicons'}) {
		...
		print '<a href="' . $link . '" ' . $href . '><img style="padding: 7px; filter: grayscale(1) contrast(0) brightness(0); width: '.$width.'px; height: '.$height.'px;" src="' . $icon . '" width="' . $width . '" height="' . $height . '">';
		print $before;
		...
		print '<img style="padding: 7px; filter: grayscale(1) contrast(0) brightness(0); width: '.$width.'px; height: '.$height.'px;" src="' . $icon . '" width="' . $width . '" height="' . $height . '">';
		print $before;
		print '<p>' . $title . '</p>';
		print $after;
		print '</div>';
...
sub theme_ui_table_start {
	...
	$rv .= "<table width=100% class='table'>\n";
...
sub theme_ui_select {
	...
	$rv .= '<select class="form-control" data-width="auto" name="' . &quote_escape($name) . '" id="' . &quote_escape($name) . '"';
...
sub theme_ui_submit {
	...
	$rv .= '<button type="submit" class="btn btn-outline-primary btn-bwtheme ' . $btntype . '" ';
...
sub theme_ui_tabs_start {
	...
	$rv .= '<ul class="nav nav-tabs bg-primary">' . "\n";
	foreach my $t (@$tabs) {
		my $tabid = "tab_".$t->[0];
		if ($t->[0] eq $sel) {
			$rv .= '<li class="active nav-items"><a data-toggle="tab" class="nav-link" href="#' . $t->[0] . '">' . $t->[1] . '</a></li>' . "\n";
		} else {
			$rv .= '<li class="nav-items"><a data-toggle="tab" class="nav-link" href="#' . $t->[0] . '">' . $t->[1] . '</a></li>' . "\n";
		}
	}
	$rv .= '</ul>' . "\n";
